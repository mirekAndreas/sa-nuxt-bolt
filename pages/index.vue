...
const venues = ref<Venue[]>([
  ...
  },
  {
    ...
    images: ["https://images.unsplash.com/photo-1519225421980-715cb0215aed?ixlib=rb-4.0.3"],
...
const isVenueValid = (venue: Venue) => {
  const now = new Date();
  const validTo = venue.subscription.validTo ? new Date(venue.subscription.validTo) : null;
...
const filteredVenues = computed(() => {
  return venues.value.filter((venue) => {
    ...
    if (filters.value.tags?.length) {
      ...
      if (!filters.value.tags.some(tagSlug => venueTagSlugs.includes(tagSlug))) {
        return false;
      }
    }
